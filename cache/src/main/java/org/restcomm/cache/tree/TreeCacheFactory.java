...
/**
 * Factory class that contains API for users to create instances of {@link org.restcomm.cache.tree.TreeCache}
 *
...
public class TreeCacheFactory {
    ...
    * @throws RuntimeException if the invocation batching configuration is not enabled.
    */
   ...
   public <K, V> TreeCache<K, V> createTreeCache(Cache<K, V> cache) {
      ...
      if (!cache.getCacheConfiguration().invocationBatching().enabled()) {
         ...
         //      " calling configurationBuilder.invocationBatching().enable()");
          ...
          throw new RuntimeException("invocationBatching is not enabled for cache '" +
                  cache.getName() + "'. Make sure this is enabled by" +
                  " calling configurationBuilder.invocationBatching().enable()");
      }
